...
#define CREATE_GROUND_COLLISION_OBJECT 1
	...
	startOffset.setValue(0,0.5,0);

	CreateBox(0, 0., 2., 0., 1., 0.2, 1.); // Create the box

	//Create the horizontal leg components
	CreateCylinderX(1,1.75,2,0.,.75,.2,.2,0.,0.,0.); // Leg 1
	CreateCylinderX(2,-1.75,2.,0.,.75,.2,.2,0.,0.,0.); //Leg 2
	CreateCylinderZ(3,0,2.,1.75,.2,.2,.75,0,0.,0.); //Leg 3
	CreateCylinderZ(4,0,2.,-1.75,.2,.2,.75,0,0.,0); //Leg 4

	//Create the vertical leg components
	CreateCylinderY(5,2.5,1,0,.2,1,.2,0,0.,0); //Leg 5
	CreateCylinderY(6,-2.5,1.,0,.2,1,.2,0,0.,0); //Leg 6
	CreateCylinderY(7,0,1.,2.5,.2,1,.2,0,0.,0); //Leg 7
	CreateCylinderY(8,0,1,-2.5,.2,1,.2,0,0.,0); //Leg 7
	clientResetScene();
}
...
void RagdollDemo::CreateBox(int index, double x, double y, double z, double length, double width, double height)
{
	...
	body[index]= localCreateRigidBody(btScalar(1), offset, geom[index]); 
}

void RagdollDemo::CreateCylinderX(int index, double x, double y, double z, double length, double width, double height, double yaw, double pitch, double roll)
{
	btTransform transform;
	transform.setIdentity();
	...
	transform.getBasis().setEulerZYX(yaw, pitch, roll);

	geom[index] = new btCylinderShapeX(btVector3(btScalar(length), btScalar(width), btScalar(height)));
	body[index] = localCreateRigidBody(btScalar(1.), offset*transform, geom[index]);
}
void RagdollDemo::CreateCylinderZ(int index, double x, double y, double z, double length, double width, double height, double yaw, double pitch, double roll)
{
	btTransform transform;
	transform.setIdentity();
	...
	geom[index] = new btCylinderShapeZ(btVector3(btScalar(length), btScalar(width), btScalar(height)));
	body[index] = localCreateRigidBody(btScalar(1.), offset*transform, geom[index]);
}
void RagdollDemo::CreateCylinderY(int index, double x, double y, double z, double length, double width, double height, double yaw, double pitch, double roll)
{
	btTransform transform;
	transform.setIdentity();
	...
	geom[index] = new btCylinderShape(btVector3(btScalar(length), btScalar(width), btScalar(height)));
	body[index] = localCreateRigidBody(btScalar(1.), offset*transform, geom[index]);
}
void RagdollDemo::DeleteObject(int index)
{
	if (body[index] && body[index]->getMotionState())
     {
         delete body[index]->getMotionState();
     }
     m_dynamicsWorld->removeCollisionObject( body[index] );
